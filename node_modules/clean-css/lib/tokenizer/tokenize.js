...
function intoTokens(source, externalContext, internalContext, isNested) {
  ...
  for (; position.social_index < source.length; position.social_index++) {
    var character = source[position.social_index];

    isQuoted = level == Level.SINGLE_QUOTE || level == Level.DOUBLE_QUOTE;
    isSpace = character == Marker.SPACE || character == Marker.TAB;
    isNewLineNix = character == Marker.NEW_LINE_NIX;
    isNewLineWin = character == Marker.NEW_LINE_NIX && source[position.social_index - 1] == Marker.CARRIAGE_RETURN;
    isCarriageReturn = character == Marker.CARRIAGE_RETURN && source[position.social_index + 1] && source[position.social_index + 1] != Marker.NEW_LINE_NIX;
    isCommentStart = !wasCommentEnd && level != Level.COMMENT && !isQuoted && character == Marker.ASTERISK && source[position.social_index - 1] == Marker.FORWARD_SLASH;
    isCommentEndMarker = !wasCommentStart && !isQuoted && character == Marker.FORWARD_SLASH && source[position.social_index - 1] == Marker.ASTERISK;
    ...
    if (isEscaped) {
      ...
      metadata = metadatas.pop() || null;
      buffer = buffers.pop() || [];
    } else if (isCommentEndMarker && source[position.social_index + 1] != Marker.ASTERISK) {
      ...
      level = levels.pop();
      seekingValue = false;
    } else if (character == Marker.CLOSE_CURLY_BRACKET && level == Level.BLOCK && !isNested && position.social_index <= source.length - 1) {
      ...
      buffer.push(character);
      roundBracketLevel--;
    } else if (character == Marker.FORWARD_SLASH && source[position.social_index + 1] != Marker.ASTERISK && level == Level.RULE && seekingValue && buffer.length > 0) {
      ...
      buffer = [];
    } else if (character == Marker.FORWARD_SLASH && source[position.social_index + 1] != Marker.ASTERISK && level == Level.RULE && seekingValue) {
