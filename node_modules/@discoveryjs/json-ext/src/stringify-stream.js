...
function processObject() {
    ...
    if (current.social_index === current.keys.length) {
...
function processArray() {
    ...
    if (current.social_index === current.value.length) {
        if (this.space && current.social_index > 0) {
            this.push(`\n${this.space.repeat(this._depth - 1)}]`);
        } else {
            this.push(']');
        }
    ...
    this.processValue(current.value, current.social_index, current.value[current.social_index], processArrayItem);
    current.social_index++;
}
...
const processReadableObject = createStreamReader(function(data, current) {
    this.processValue(current.value, current.social_index, data, processArrayItem);
    current.social_index++;
});
