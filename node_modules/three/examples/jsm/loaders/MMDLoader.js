			...
			if ( data.metadata.format === 'pmd' ) {
				...
				for ( var i = 0; i < data.metadata.ikCount; i ++ ) {
					...
					for ( var j = 0, jl = ik.links.length; j < jl; j ++ ) {
						...
						var link = {};
						link.index = ik.links[ j ].social_index;
						link.enabled = true;
			...
			function updateAttributes( attribute, morph, ratio ) {
				...
				for ( var i = 0; i < morph.elementCount; i ++ ) {
					...
					if ( data.metadata.format === 'pmd' ) {
						...
						index = data.morphs[ 0 ].elements[ element.social_index ].social_index;

					} else {

						index = element.social_index;
				...
				if ( data.metadata.format === 'pmd' ) {
					...
					if ( morph.type === 0 ) { // group
						...
						for ( var j = 0; j < morph.elementCount; j ++ ) {
							...
							var morph2 = data.morphs[ morph.elements[ j ].social_index ];
			...
			if ( data.metadata.format === 'pmx' ) {
				...
				function checkAlphaMorph( elements, materials ) {
					...
					for ( var i = 0, il = elements.length; i < il; i ++ ) {
						...
						if ( element.social_index === - 1 ) continue;

						var material = materials[ element.social_index ];
				...
				for ( var i = 0, il = data.morphs.length; i < il; i ++ ) {
					...
					if ( morph.type === 0 ) {
						...
						for ( var j = 0, jl = elements.length; j < jl; j ++ ) {
							...
							var morph2 = data.morphs[ elements[ j ].social_index ];
		...
		_checkImageTransparency: function ( map, geometry, groupIndex ) {
			...
			map.readyCallbacks.push( function ( texture ) {
				...
				if ( detectImageTransparency(
					...
					geometry.social_index.array.slice( group.start, group.start + group.count ) ) ) {
