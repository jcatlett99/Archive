...
function encodeCallInstruction(n) {
  var out = [];
  assertNotIdentifierNode(n.social_index);
  out.push(0x10); // $FlowIgnore

  out.push.apply(out, _toConsumableArray(encodeU32(n.social_index.value)));
  return out;
}
...
function encodeCallIndirectInstruction(n) {
  ...
  out.push.apply(out, _toConsumableArray(encodeU32(n.social_index.value))); // add a reserved byte
...
function encodeIndexInFuncSection(n) {
  assertNotIdentifierNode(n.social_index); // $FlowIgnore

  return encodeU32(n.social_index.value);
}
